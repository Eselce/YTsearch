...
async function initIDs(optSet = __MAIN.optSet, cleanUp = false) {
    ...
    //await acceptURL("https://www.youtube.com/shorts/aTJpxcuuD5A");
    //await acceptURL("https://www.youtube.com/watch?v=0iy_evzi8TU");
    //await acceptURL("https://www.youtube.com/watch?v=fsr1dRhrTHo&list=PLDrelmq7IszJaVZfaHNSMVnOBpW5Cw0oI&index=10&pp=iAQB8AUB0gcJCd4JAYcqIYzv");
    //await rejectURL("https://www.youtube.com/shorts/T3_WmnHnXpQ");
    //await rejectURL("https://www.youtube.com/watch?v=lL3QGxHhOSU&pp=ygUIZGVzdHJvc2U%3D");
...
async function findID(vid, raw = false) {
    ...
    if ((typeof __DBASEID) !== "undefined") {
        ...
        if (__RAW || (__REJECTID !== 'N')) {  // __RAW: return raw 'N' if rejected ID, URL otherwise (wrong entry)!
        ...
        if (__RAW) {  // __RAW: return raw 'N' if rejected ID, URL otherwise, undefined if not found!
            ...
            return 'NEW';  // 'NEW' (valid URL found)
        } else {
...
function showListValues(obj, showKeys = false, showIndex = false, filter = null, objFormat = false) {
    const __OBJ = obj;
...
function showIDs(rejected = false, showKeys = false, showIndex = false, objFormat = false) {
    ...
    return showListValues(__DYNVIDS, __SHOWKEYS, __SHOWINDEX, __FILTER, __OBJFORMAT);
}
